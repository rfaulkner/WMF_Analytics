...
class SummaryReportingLoader(DataLoader):
    ...
    def process_kwargs(self, kwargs_dict):
        ...
        return min_views, one_step, country
...
class IntervalReportingLoader(DataLoader):
    ...
    def run_query(self, start_time, end_time, interval, metric_name, campaign, **kwargs):                
        ...
        if self.get_one_step_banners(start_time, end_time, campaign) or one_step_var:
            ...
            metrics_1, times_1, results_1 = self.run_query_base(start_time, end_time, interval, metric_name, campaign, query_name + '_1S', **kwargs)
            
            self._was_run_ = False
            metrics_2, times_2, results_2 = self.run_query_base(start_time, end_time, interval, metric_name, campaign, query_name, **kwargs)
            ...
            metrics, times, results = self.run_query_base(start_time, end_time, interval, metric_name, campaign, query_name, **kwargs)
    ...
    """
    def run_query_base(self, start_time, end_time, interval, metric_name, campaign, query_name, **kwargs):
        ...
        if not(self._was_run_):
            ...
            start_time = MySQLdb._mysql.escape_string(str(start_time).strip())
            end_time = MySQLdb._mysql.escape_string(str(end_time).strip())
            interval = int(MySQLdb._mysql.escape_string(str(interval)))
...
class CampaignReportingLoader(DataLoader):
    ...
    def query_totals(self, params):
        ...
        """ Resolve parameters """
        metric_name = MySQLdb._mysql.escape_string(str(params['metric_name']))
        start_time = MySQLdb._mysql.escape_string(str(params['start_time']).strip())
        end_time = MySQLdb._mysql.escape_string(str(params['end_time']).strip())
    ...
    def query_live_landing_pages(self, start_time, end_time, **kwargs):
        ...
        if 'min_donation' in kwargs:
            ...
            if not(isinstance(kwargs['min_donation'], int)):
                min_donation = 0
            else:
                min_donation = MySQLdb._mysql.escape_string(str(kwargs['min_donation']))
                min_donation = int(min_donation)
        
        if 'view_order' in kwargs:
            view_order_str = MySQLdb._mysql.escape_string(str(kwargs['view_order']))
